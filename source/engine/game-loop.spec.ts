...
describe( 'game loop', () => {
	...
	it( 'tracks total elapsed time (quantized)', () => {
		...
		const elapsed = () => update.mock.lastCall[ 1 ]
	...
	it( 'limits pending updates', () => {
		...
		expect( update.mock.calls.length ).toBe( 10 ) // loop stops at 10
	} )
	...
	it( 'resumes updates where it left off', () => {
		...
		loop.paused = false
		loop.advance( 2/60 + error )
		expect( update.mock.calls.length ).toBe( 1 ) // used to be 2
	} )
	...
	it( 'passes correct dt/t when stepping', () => {
		...
		const [ dt, elapsed ] = update.mock.lastCall
	...
	it( 'calls draw() for every animation frame', () => {
		...
		expect( draw.mock.calls.length ).toBe( 2 )
	} )
	...
	it( 'passes time since last update to draw()', () => {
		...
		const ahead = () => draw.mock.lastCall[ 0 ]

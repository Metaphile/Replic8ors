...
function programNonsense(replicator) {
  ...
  Replic8or.syncSymmetricWeights(
    replicator.brain.neurons,
    replicator.numBodySegments,
    neuronsPerSegment,
  );
}
...
Replic8or.syncSymmetricWeights = (neurons, numSegments, numNeuronsPerSegment) => {
  ...
  for (let currSegmentIndex = 1; currSegmentIndex < numSegments; currSegmentIndex++) {
    ...
    for (
      let currNeuronIndex = firstNeuronIndexCurrSegment;
      currNeuronIndex <= lastNeuronIndexThisSegment;
      currNeuronIndex++
    ) {

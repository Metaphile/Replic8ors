...
export default function Scenario(world, opts = {}) {
  ...
  function addNumBlues(howMany) {
    ...
    createPopulation(howMany, geneBank.blue, replicateReplicator, () =>
      createReplicator("blue"),
    ).forEach((blue) => {
      ...
      const tau = Math.PI * 2;
      const minAngle =
        (ownSpokeOffset + blueSpokeIndex) * (tau / numSpokes) -
        radiusScale * Math.PI * 0.5 -
        tau / numSpokes / 2;
      const maxAngle =
        (ownSpokeOffset + blueSpokeIndex) * (tau / numSpokes) -
        radiusScale * Math.PI * 0.5 +
  ...
  function addNumGreens(howMany) {
    ...
    createPopulation(howMany, geneBank.prey, replicateReplicator, () =>
      createReplicator("prey"),
    ).forEach((prey) => {
      const radiusScale = Math.pow(Math2.random(), 1 / Math.PI);
      const radius = radiusScale * world.radius;
      ...
      const tau = Math.PI * 2;
      const minAngle =
        (ownSpokeOffset + preySpokeIndex) * (tau / numSpokes) -
        radiusScale * Math.PI * 0.5 -
        tau / numSpokes / 2;
      const maxAngle =
        (ownSpokeOffset + preySpokeIndex) * (tau / numSpokes) -
  ...
  function addNumReds(howMany) {
    ...
    createPopulation(howMany, geneBank.predator, replicateReplicator, () =>
      createReplicator("predator"),
    ).forEach((predator) => {
      const radiusScale = Math.pow(Math2.random(), 1 / Math.PI);
      const radius = radiusScale * world.radius;
      ...
      const tau = Math.PI * 2;
      const minAngle =
        (ownSpokeOffset + predatorSpokeIndex) * (tau / numSpokes) -
        radiusScale * Math.PI * 0.5 -
        tau / numSpokes / 2;
      const maxAngle =
        (ownSpokeOffset + predatorSpokeIndex) * (tau / numSpokes) -

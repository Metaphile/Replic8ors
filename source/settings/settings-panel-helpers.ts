...
export function attachInputHandler(fieldDef, element) {
  ...
  function applyValue(otherInputSelector) {
    return function (event) {
      ...
      if (input.value != defaultSettings[fieldDef.section][fieldDef.settingsKey]) {
        label?.classList.add("modified");
      } else {
        label?.classList.remove("modified");
      }

      const value = round(input.value);
      if (other) other.value = value;

      if (!fieldDef.validator(input.value)) {
  ...
  element
    .querySelectorAll(numberInputSelector)
    .forEach((el) => el.addEventListener("change", applyValue(rangeInputSelector)));
  element
    .querySelectorAll(rangeInputSelector)
    .forEach((el) => el.addEventListener("input", applyValue(numberInputSelector)));
}

...
export default function SettingsPanel() {
  ...
  const minReplicatorsFieldDef = {
    ...
    description:
      "If the population falls below this number, clones of past members will be added (max population takes precedence)",
    step: 1,
    rangeInputMinValue: 0,
  ...
  for (const fieldDef of predatorFieldDefs) {
    if (fieldDef.fieldLabel) {
      predatorFields.push(helpers.populateFieldTemplate(fieldDef, settings, defaultSettings));
    } else {
  ...
  for (const fieldDef of preyFieldDefs) {
    if (fieldDef.fieldLabel) {
      preyFields.push(helpers.populateFieldTemplate(fieldDef, settings, defaultSettings));
    } else {
      preyFields.push("<hr>");
    }
  }
  ...
  for (const fieldDef of blueFieldDefs) {
    if (fieldDef.fieldLabel) {
      blueFields.push(helpers.populateFieldTemplate(fieldDef, settings, defaultSettings));
    } else {
      blueFields.push("<hr>");
    }
  }

...
function drawGauge(ctx, neuron) {
  ...
  if (!neuron.firing) {
    ...
    const inhibitoryStop = Math.max(
      gaugeStart - neuron.inhibitoryInput * tau,
      -(tau - potentialStop),
    );
...
NeuronView.prototype = {
  update(dt_real, dt_sim) {
    ...
    if (this.neuron.firing)
      this.radius += jiggle(Math.max(1 - this.neuron.potential, Number.MIN_VALUE));

    if (!this.neuron.firing) {
      this.potentialDecayRotation -=
        potentialDecayFn(this.neuron.potentialDecayRate) * Math.PI * 2 * dt_sim;

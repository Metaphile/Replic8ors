...
ReplicatorView.prototype = {
  ...
  update(dt_real, dt_sim) {
    ...
    this._apparentEnergy +=
      (this.replicator.energy - this._apparentEnergy) * 9 * Math.min(dt_sim, dt_real);
  ...
  drawInputSignal(ctx, inputNode, neuronView) {
    const origWeight =
      this.replicator.ancestorWeights[neuronView.neuron.index].weights[neuronView.neuron.index];
      ...
      for (let j = i + 1; j < numViews; j++) {
        ...
        if (drawAllSignals || drawTheseSignals) {
          if (leftView.neuron.firing) {
            const dimIgnoredSignal =
              rightView.neuron.firing && !leftView.active && !rightView.active;
          ...
          if (rightView.neuron.firing) {
            const dimIgnoredSignal =
              leftView.neuron.firing && !leftView.active && !rightView.active;
  ...
  drawInternalSignal(ctx, sourceView, targetView) {
    const origWeight =
      this.replicator.ancestorWeights[targetView.neuron.index].weights[sourceView.neuron.index];
  ...
  drawSignal(ctx, sourceNode, targetNode, signalParts, txProgress, offsetEndpoints = true) {
    ...
    for (const part of signalParts) {
      const partStartLength =
        connLength * txProgress2 + part.start * connLength * (1 - txProgress2);
  ...
  drawFlippers(ctx) {
    ...
    for (let i = 0; i < replicator.flippers.length; i++) {
      ...
      const flipperBase = Vector2(
        Math.cos(flipper.angle) * (replicator.radius - 1.3),
        Math.sin(flipper.angle) * (replicator.radius - 1.3),
      );
  ...
  draw(ctx, camera, detail, keepHoverOverride) {
    ...
    if (this.effects.death) {
      ctx.beginPath();
      ctx.arc(
        this.replicator.position.x,
        this.replicator.position.y,
        this.replicator.radius,
        0,
        Math.PI * 2,
      );

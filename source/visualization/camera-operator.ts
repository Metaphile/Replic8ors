...
export default function CameraOperator(camera, canvas) {
  ...
  self.smoothZoom = (zoomDelta, zoomX, zoomY) => {
    ...
    const newZoom = Math2.clamp(oldZoom * (1 + zoomDelta), MIN_ZOOM, MAX_ZOOM);

    const oldCenter = camera.viewCenter(canvas);
    camera.zoomTo(newZoom, zoomX, zoomY);
    const newCenter = camera.viewCenter(canvas);
  ...
  self.update = (dt_real, dt_sim) => {
    ...
    if (true) {
      ...
      const px = targetX + self.offset.x;
      const py = targetY + self.offset.y;

      // adjust position more aggressively while zooming
      // var speed = 5.1 * ( 1 + Math.abs( this._zoomBuffer * 1.1 ) )
      const speed = 5.6;

      camera.pan((px - viewCenter.x) * speed * dt_real, (py - viewCenter.y) * speed * dt_real);
    }
